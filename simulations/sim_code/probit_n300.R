...
out = foreach::foreach(r = 1:nReplicate, .combine = 'mycombine', .multicombine = T, 
  ...
  data_all = data_model(n=n, m=m, p=p,true.alpha = true.alpha, 
                           true.beta = true.beta, mu_eps = mu_eps, C.param = c(1, 0.2))
...
out3 = c(list(n=n, m=m, p=p, true.beta = true.beta), out)
filename.rdata = paste0("out_probit_n", n, ".Rdata") 
save(out3, file = filename.rdata)

...
load("simulations/sim_mcmc_output/out_poly_misspec_n50.Rdata")
load("simulations/sim_mcmc_output/out_poly_misspec_n100.Rdata")
load("simulations/sim_mcmc_output/out_poly_misspec_n300.Rdata")
load("simulations/sim_mcmc_output/out_poly_misspec_n500.Rdata")
...
g.mse.beta = ggplot(df.mse.beta, aes(x = as.factor(n), y = mse, col = method)) + 
  ...
  scale_color_manual(values = c( "dodgerblue", "grey40","orange", "sienna"), 
                     labels = c('GP-DP', 'BP-DP', 'GP-N', 'BP-N'))+
  xlab("n") + ylab("Mean\nsquared error") + coord_cartesian(ylim = c(0,0.005)) + themegg 
g.mse.beta
...
ggsave("simulations/sim_plots/figures/simpolymisspec.pdf", g.all, width = 7, height = 9)
